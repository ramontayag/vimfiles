...
h2. Adding submodules

If you want to add submodules, do something like

pre. git submodule add git://path.to/repo.git bundle/directory_name

Doing the above won’t install it yet though, you’ll need to call @git submodule update --init@ the same way you’re told to when you first install vimfiles.
